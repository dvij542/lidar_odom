...
files = ['traj_files/vicon-data-clean/vicon-circle-data-20240410-021643.csv']
# files = ['traj_files/vicon-data-clean/vicon-circle-data-20240316-024251.csv']
# files = ['traj_files/vicon-data-clean/vicon-circle-data-20240316-234006.csv']
...
parser.add_argument('--lf', required=False, default=0.11,help='Specify the distance from COM to front axle')
parser.add_argument('--lr', required=False, default=0.23,help='Specify the distance from COM to rear axle')
parser.add_argument('--theta_bias', required=False, default=-0.0,help='Specify the theta bias')
parser.add_argument('--steer_bias', required=False, default=0.03,help='Specify the steer bias')
parser.add_argument('--steer_factor', required=False, default=0.36/0.8,help='Specify the steer factor')
parser.add_argument('--filter_v', required=False, default=4,help='Exp filter for velocities')
parser.add_argument('--filter_theta', required=False, default=3,help='Exp filter for yaw')
parser.add_argument('--filter_a', required=False, default=1,help='Exp filter for accelerations')
parser.add_argument('--filter_wdot', required=False, default=1,help='Exp filter for wdot')
...
for file_path in files:
    ...
    path.append([path_file['pos_x'][0],path_file['pos_y'][0],path_file['yaw'][0],path_file['target_steer'][0],path_file['target_vel'][0],path_file['time'][0]])
    for i in range(1,len(path_file)) :
        mask_partition.append(0)
        path.append([path_file['pos_x'][i],path_file['pos_y'][i],path_file['yaw'][i],path_file['target_steer'][i],path_file['target_vel'][i],path_file['time'][i]])
path = np.array(path)
...
steers_ = xyts[:,-3]*steer_factor + steer_bias
throttles = xyts[:,-2]

steer = steer_bias
steers = np.array(steers_)
for i in range(DELAY,len(steers_)) :
    steer += (steers_[i-DELAY]-steer)/filter_steer
    steers[i] = steer
steers[:DELAY] = 0.

# NOTE: We are taking theta at t
midts = xyts[:-1,2]
# NOTE: Vel yaw calculated at time t
...
mask = (1-mask1)*mask2 + mask3 + mask_partition[1:] + (dts>dt+0.005)
for i in range(mask_padding) :
    mask[1:] += mask[:-1]
    mask[:-1] += mask[1:]
mask = mask > 0
xy_dots[:,1] += t_dots[:,0]*(lr+ld)
vx = 0.
vy = 0.
w = 0.
times = dt*np.arange(len(xyts))
for i in range(0,len(xy_dots)) :
    if mask[i]*0 == 0 :
...
if PLOT_SLIP_ANGLES :
    ...
    # print(steers[:-1])
    plt.plot(times[:-1],10*steers[:-1],label='steers')
    plt.legend()
    plt.show()
...
if PLOT_ACCS :
    ...
    plt.plot(times[:-1],xy_dots[:,0]*t_dots[:,0],label='ay from vw')
    plt.plot(times[1:-1],t_dot_dots[:,0],label='w_acc')
    plt.legend()
    plt.show()
...
for i in tqdm.tqdm(range(len(xy_dot_dots))) :
    ...
    A = np.array([[1,-math.sin(delta),0.,xy_dots[i,1]*t_dots[i,0]],\
                 ...
                 [0.,lf*math.cos(delta)/(0.15*0.15),-lr/(0.15*0.15),0.],\
                 [0.,0.,0.,1.]])
    ...
    if mask[i]==0 and abs(steers[i])>0.1 and xy_dots[i,0]>1. :#and alpha_f[i] > 0.05 and alpha_f[i] < 0.2:
        n_included += 1
        Frxs.append(X[0,0])
        Ffys.append(X[1,0])
        Frys.append(X[2,0])
...
if PLOT_GG :
    plt.scatter(np.array(alpha_fs),Ffys,s=3,label='front tire calculated Fy/m')
    plt.scatter(np.array(alpha_rs),Frys,s=3,label='rear tire calculated Fy/m')
...
inputs = np.array([throttles[1:-1],steers[1:-1]]).T
print('total points taken',100.*n_included/n_total)
if args.save:
