...
def load_data(file_name):
    ...
    y_all = (data_dyn['states'].T[:,RES:]-data_dyn['states'].T[:,:-RES])/RES
    print(data_dyn['inputs'].shape)    
    print(data_dyn['states'].shape)    
    x = np.concatenate([
        data_dyn['inputs'].T[:,(RES):].T,
        data_dyn['inputs'].T[1,(RES):].reshape(1,-1).T,
...
K = 0
t1 = time.time()
for i in range(0,len(x_train)-100-ignore_first) :
    # Zero your gradients for every batch!
    optimizer.zero_grad()
    # model = model.cuda()
    # x_train = x_train.cuda()
    # y_train = y_train.cuda()
    outputs = model(x_train[ignore_first+i:ignore_first+i+100],debug=False)
    # if i==4999 :
    ...
    # diff_vx = np.abs(x_train[ignore_first+i:ignore_first+i+100,3:4]-x_train[ignore_first-1+i:ignore_first-1+i+100,3:4])
    # mask = (x_train[ignore_first:,3:4]>min_v)*(x_train[ignore_first:,3:4]<max_v)*(diff_vx<1.5)
    # mask = mask.cuda()
    loss = loss_fn(outputs[:,1:]*mask[ignore_first+i:ignore_first+i+100,:], y_train[ignore_first+i:ignore_first+i+100,1:]*mask[ignore_first+i:ignore_first+i+100,:])
    loss.backward()
    diffs.append(float(torch.sum(outputs[:,1:2]*mask[ignore_first+i:ignore_first+i+100,:]-y_train[ignore_first+i:ignore_first+i+100,1:2]*mask[ignore_first+i:ignore_first+i+100,:])))
    print(diffs[-1])
    print("Iter " + str(i) + " loss1 : ", loss.item(), " Fy : ", model.Fy.weight.data.item(), " Ry : ", model.Ry.weight.data.item())
    Kfs.append(model.Fy.weight.data.item()+K*math.exp(-10*i/len(x_train)))
    Krs.append(model.Ry.weight.data.item()+K*math.exp(-10*i/len(x_train)))
    # Adjust learning weights
    optimizer.step()
    
t2 = time.time()
print("Average training time : ", (t2-t1)/(len(x_train)-100-ignore_first))
print("Training done")
end = time.time()
...
# plt.show()
alpha_f = torch.tensor(np.arange(-.25,.25,0.004)).unsqueeze(1)#.cuda()
Ffy = model.get_force_F(alpha_f).cpu().detach().numpy()
...
alpha_r = torch.tensor(np.arange(-.25,.25,0.004)).unsqueeze(1)#.cuda()
Fry = model.get_force_R(alpha_r).detach().cpu().numpy()
...
plt.plot(Kfs,label='Kf')
plt.plot(Krs,label='Kr')
