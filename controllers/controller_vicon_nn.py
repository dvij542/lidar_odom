...
MPPI_STEER_PROJ = -0.22
# 。45 for clockwise, .48 for counter
MPPI_STEER_SHIFT = 0.1
DELAY = 3
...
RECOVER_COUNTDOWN = 2
WITHOUT_VICON = False
online_dataset_states = [] # Only vx,vy,w
online_dataset_cmds = [] # Only delta,throttle
AUGMENT = 'SYMMETRIC'
...
model = SimpleModel(5,10,3).double()
# Define loss function and optimizer
criterion = nn.MSELoss()
optimizer = optim.SGD(model.parameters(), lr=0.01)


def pure_pursuit(obs, goal):
    x = obs[0]
    y = obs[1]
    psi = obs[2]
    xt = goal[0]
    yt = goal[1]
    l = LF + LR
    L = np.sqrt((xt - x)**2 + (yt - y)**2)
    xd_ = xt - x
    yd_ = yt - y
    xd = xd_ * np.cos(psi) + yd_ * np.sin(psi)
    yd = -xd_ * np.sin(psi) + yd_ * np.cos(psi)
    steer = 2 * l * yd / L**2
    return steer


def train_step(states,cmds,augment=False) :
    global model, ratio
    X = np.concatenate((np.array(states),np.array(cmds)),axis=1)[:-1]
    Y = 5.*(np.array(states)[1:] - np.array(states)[:-1])
    ...
    for j in range(5) :
        X_[:,:3] += model(X_)
    w_final = X_[0,2]
    w_expected = X_[0,0]*torch.tan(X_[0,4]*0.5)/(LF+LR)
    ratio = max(1.,float(w_final/w_expected))
...
if 'mppi' in controller_type:
    ...
    if 'nn-phyx-kbm' in controller_type:
        ...
        model_params = DynamicParams(num_envs=N_ROLLOUTS, DT=DT,Sa=SA, Sb=-0., K_FFY=Kf, K_RFY=Kr, Ta=8., Tb=-1.)
    ...
    ,
        ...
        rollout_start_fn=fn if 'kbm' not in controller_type else dynamics.reset,
        debug=False,
        fix_history='phyx-kbm' in controller_type,
        num_obs=dynamics.obs_dim,
        num_actions=2,
    )
...
###################### Logging dir ##################
DATA_DIR = '/home/cmu/catkin_ws/offroad/data/dataset2'
...
def update_vicon():
    ...
    while True :
        ...
        pose_x = -unpacked_data[1]/1000.
        pose_y = unpacked_data[0]/1000.
        pose_yaw = unpacked_data[2] - np.pi #- 0.1
...
def reference_traj(t,d=False):
    if trajectory_type == 'circle':
        ...
        angle = -np.pi/2  + x_radius * SPEED * t
        if d :
            print(np.arctan2(np.sin(angle),np.cos(angle)))
...
def joystick_controller():
    global pub_x, pub_y, pub_yaw, pub_vx, pub_vy, pub_w, pub_steer, pub_throttle, online_dataset_states, online_dataset_cmds
    ...
    omega = 0.
    while True:
        global traj, pub, car_state, prev_x, prev_y, prev_psi, velocity, time_complement, RECOVOER_TIME
        now = rospy.Time.now()
        cmd = Joy()
        ...
        vy_ = omega*(ld+LR) + v_vec[1] * np.cos(car_state['vicon_loc'][2]) - v_vec[0] * np.sin(car_state['vicon_loc'][2])
        ...
        pose.pose.position.x = car_state['vicon_loc'][0]
        pose.pose.position.y = car_state['vicon_loc'][1]
        quaternion = euler_to_quaternion(0., 0., car_state['vicon_loc'][2])
        ...
        pub_vy.publish(vy)
        pub_w.publish(omega)
        pub_x.publish(car_state['vicon_loc'][0])
        pub_y.publish(car_state['vicon_loc'][1])
        # pub_yaw.publish(car_state['vicon_loc'][2])
        pub_yaw.publish(ratio)
        pub_steer.publish(car_state['steering'])
        pub_throttle.publish(car_state['throttle'])
        
        online_dataset_states.append([vx,vy,omega])
        online_dataset_cmds.append([car_state['throttle'],car_state['steering']])
        
        if len(online_dataset_cmds) > ONLINE_BUFFER_SIZE :
            online_dataset_states = online_dataset_states[-ONLINE_BUFFER_SIZE:]
            online_dataset_cmds = online_dataset_cmds[-ONLINE_BUFFER_SIZE:]
            train_step(online_dataset_states,online_dataset_cmds,augment=True)
        
        if MODIFY_MODEL :
            dynamics.batch_Sa[:] = SA*ratio
        # Publish path
        ...
        if 'random_walk' in controller_type:
            ...
            if pid_recover:
                ...
                target_pos_list = [reference_traj(t_closed + i*DT,d=(i==0)) for i in range(H+DELAY)]
            ...
            t_idx = np.argmin(distance_list)
            t_closed = waypoint_t_list[t_idx]
            target_pos_list = [reference_traj(waypoint_projection_shift_time + t_closed + i*DT*.75,d=(i==0)) for i in range(H+DELAY+1)]
        ...
        car_state['steering'] += K_steer*(action[1] - car_state['steering'])*DT
        
        
        ################### Publish Command to ROS #############################
        ros_command_steering = steering_to_ros_command(action[1], calibrate=False)
...
def publish_path() :
    ...
    while not rospy.is_shutdown():
        ...
        for t in np.arange(0.,4.,0.1):
            ...
            pose.header.stamp = rospy.Time.now()
            pose.header.frame_id = 'map'
            x,y = reference_traj(t)[0], reference_traj(t)[1]
            pose.pose.position.x = x
            pose.pose.position.y = y
            pose.pose.orientation.w = 1.0
            path_msg.poses.append(pose)

        ttl_pub.publish(path_msg)
        rate.sleep()
...
if __name__ == '__main__':
    ...
    pub_x = rospy.Publisher('/state/x', Float32, queue_size=1)
    pub_y = rospy.Publisher('/state/y', Float32, queue_size=1)
    pub_yaw = rospy.Publisher('/state/yaw', Float32, queue_size=1)
    pub_vx = rospy.Publisher('/state/vx', Float32, queue_size=1)
    pub_vy = rospy.Publisher('/state/vy', Float32, queue_size=1)
    pub_w = rospy.Publisher('/state/w', Float32, queue_size=1)
    pub_steer = rospy.Publisher('/cmds/steer', Float32, queue_size=1)
    pub_throttle = rospy.Publisher('/cmds/throttle', Float32, queue_size=1)
    pub_pose = rospy.Publisher('/pose', PoseStamped, queue_size=1)
    ttl_pub = rospy.Publisher('/ttl', Path, queue_size=1)
    traj_pub = rospy.Publisher('/traj', Path, queue_size=1)
    # path_msg = Path()
    # path_msg.header.stamp = rospy.Time.now()
    # path_msg.header.frame_id = 'map'
    ...
    ttl_thread = threading.Thread(target=publish_path)
    # ttl_thread.start()

    traj_thread = threading.Thread(target=publish_traj)
    # traj_thread.start()
    ...
    try:
        ...
        # traj_thread.join()
        vicon_thread.join()
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!vicon completed")
        if 'teleop' in controller_type or 'random_walk' in controller_type:
            keyboard_thread.join()
        joystick_thread.join()
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!joystick completed")
        # vicon_thread.join()