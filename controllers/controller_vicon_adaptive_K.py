...
class ResidualModel(torch.nn.Module):
    def __init__(self, deltat = DT):
        ...
        self.Ry.weight.data.fill_(Kf)
        
        self.Fy = torch.nn.Linear(1,1,bias=False).to(torch.float64)
        self.Fy.weight.data.fill_(Kr)
    ...
    def forward(self, x, debug=False,n_divs=2):
        ...
        for i in range(n_divs) :
            ...
            w_dot = (Ffy*LF*torch.cos(theta)-Fry*LR)/(0.15*0.15)
...
if 'mppi' in controller_type:
    ...
    if 'nn-phyx-kbm' in controller_type:
        ...
        model_params = DynamicParams(num_envs=N_ROLLOUTS, DT=DT,Sa=0.36/0.9, Sb=-0., K_FFY=Kf, K_RFY=Kr, Ta=8., Tb=-1.)
...
def pure_pursuit(obs, goal, L=LF+LR):
    x, y, psi = obs
    xg, yg = goal
    delta = np.arctan2(yg - y, xg - x) - psi
    while delta > np.pi:
        delta -= 2 * np.pi
    while delta < -np.pi:
        delta += 2 * np.pi
    dist = np.sqrt((xg - x)**2 + (yg - y)**2)
    dx = dist*np.cos(delta)
    dy = dist*np.sin(delta)
    steer = 2 * L * dy / (dist**2)
    return steer
...
def joystick_controller():
    ...
    while True:
        ...
        vx += (vx_ - vx) / K_vel
        vy += (vy_ - vy) / K_vel
        buffer_vs.append([vx,vy,omega])
        if len(buffer_vs) > RES+1 :
            buffer_vs = buffer_vs[-(RES+1):]
        print(vx,vy,omega)
        ...
        steer_exp = omega/car_state['throttle']
        cond = (car_state['throttle']<0.15 or ((speed>min_v) and (speed<max_v))) \
            and (omega*car_state['steering']>0.) and (steer_exp<max_w_cmd) and (steer_exp>-max_w_cmd)
        if cond : 
            x_train.append([car_state['throttle'],SA*car_state['steering'],SA*car_state['steering'],vx,vy,omega])
            y_train.append([(buffer_vs[-1,0]-buffer_vs[0,0])/RES,(buffer_vs[-1,1]-buffer_vs[0,1])/RES,(buffer_vs[-1,2]-buffer_vs[0,2])/RES])
        if len(x_train) > TRAIN_SIZE :
            x_train = x_train[-TRAIN_SIZE:]
            y_train = y_train[-TRAIN_SIZE:]
            # Train loop
            optimizer.zero_grad()
            outputs = model(torch.tensor(x_train),debug=False)
            loss = loss_fn(outputs[:,1:], torch.tensor(y_train)[:,1:])
            loss.backward()
            optimizer.step()
        
        print("Pred params (Kf,Kr)",model.Fy.weight.data.item(),model.Ry.weight.data.item())
        
        if MODIFY_MODEL :
            dynamics.batch_K_FFY[:] = float(model.Fy.weight.data.item())
            dynamics.batch_K_RFY[:] = float(model.Ry.weight.data.item())
